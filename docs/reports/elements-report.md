...
----|-----|------|---------|-------------|-------|------
 ...
 :ddd/c-open-host-service | Open Host Service | :concept |  | An open protocol that gives access to your subsystem as a set of services. |  | #{"Pattern"}
 :ddd/c-factories | Factories | :concept |  | A domain object for the creation of complex domain objects which enforce the invariants of these objects on creation. |  | #{"Pattern"}
 :ddd/c-layered-archtecture | Layered Architecture | :concept |  | Isolates the expression of the domain model and the business logic, and eliminates any dependency on infrastructure, user interface, or application logic that is not business logic. |  | #{"Pattern"}
 :ddd/c-hands-on-modellers | Hands On Modellers | :concept |  | Developers must be involved with the model and have contact with domain experts. |  | #{"Pattern"}
 :ddd/c-domain-vision-statement | Domain Vision Statement | :concept |  | A short description of the core domain and the value it will bring. |  | #{"Pattern"}
 ...
 :ddd/c-highlighted-core | Highlighted Core | :concept |  | Describes the core elements of the domain and their interactions. |  | #{"Pattern"}
